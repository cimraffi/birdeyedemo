...
console.log("constructor 111");
  ...
  updatePosition_(position) {
     console.log("updatePosition_"+this.position_modify);
     var newPosition = new google.maps.LatLng(position.coords.latitude,
           position.coords.longitude);
     if (this.position_modify){
            console.log("updatePosition_11111");
         var ll = coordtransform.wgs84togcj02(position.coords.longitude, position.coords.latitude);
         newPosition = new google.maps.LatLng(ll[1], ll[0]);
     }

     var mapNotSet = this.marker_.getMap() == null;
     ...
     if (this.position_callback) {
         this.position_callback(newPosition);
     }
  }
